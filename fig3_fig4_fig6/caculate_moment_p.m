function [mean,  fano,   skewness, kurtosis, HD, y]=caculate_moment_p(parameter,model)
     ...
     elseif model==5
        la=parameter(1); ga=parameter(2); v=parameter(3); mu=parameter(4);
        N = 2*M+10; d=1; num = 2*N;
        Q = zeros(num);
        one = ones(num,1);
        for i = 1:N-1
     ...
     elseif model == 6
        la=parameter(1); ga=parameter(2); v=parameter(3); nu=parameter(4);
        N = 2*M+10; d=1; num = 2*N;
        Q = zeros(num);
        one = ones(num,1);
        for i = 1:N-1
